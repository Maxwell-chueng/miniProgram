...
Page({
  ...
  async onReachBottom(){
        ...
        if(Math.ceil(this.totalPage / this.requestParams.pagesize) >  this.requestParams.pagenum){
            ...
            let goodListDatas = await request('https://api-hmugo-web.itheima.net/api/public/v1/goods/search','GET',this.requestParams);
  ...
  handleTab(e){
      ...
      let {tab,goodListData,initialData} = this.data;
      ...
      if(currentIndex ===2){
        ...
        this.setData({
          tab,goodListData:initial,currentIndex
        });
      }
  },
  async getListData(){
      let goodListData = await request('https://api-hmugo-web.itheima.net/api/public/v1/goods/search','GET',this.requestParams);
      if(goodListData.statusCode === 200){
          ...
          let initialData = JSON.parse(JSON.stringify(goodListData));
          this.setData({
            goodListData,
            initialData
          });
